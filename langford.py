...
def langford_data_structure(N):
    ...
    for v in range(1, N+1):
        ...
        for pos in range(2*N-1):
            cad = cadValor + "p" + str(pos)

            if pos + v + 1 <= 2*N - 1:
                
                posiciones = Y.get(cad,[])
                if len(posiciones) == 0:
                    Y[cad] = [ value(v), position(pos), position(pos+(v+1))]
                else:
                    posiciones.append([ value(v), position(pos), position(pos+(v+1))])
                    Y[cad] = posiciones
